# Harness Engineering 实践教程：让 AI Agent 可控产出的工程方法
...
- [2.1 先约束环境，再让 AI 写代码](#21-先约束环境再让-ai-写代码)
- [2.2 知识写成地图，不写成百科](#22-知识写成地图不写成百科)
- [2.3 约束必须能执行](#23-约束必须能执行)
- [2.4 上下文冲突了才拆 Agent](#24-上下文冲突了才拆-agent)
- [2.5 让机器做追踪](#25-让机器做追踪)
- [2.6 Harness 是被失败逼出来的](#26-harness-是被失败逼出来的)

**[3. 怎么开始](#3-怎么开始)** — Checklist、成本、结语
- [3.1 最小 Harness](#31-最小-harness)
- [3.2 成本与局限](#32-成本与局限)
- [3.3 结语](#33-结语)
...
墨简用 7 个专职 Agent 组成链路。实际运行时暴露了一个系统性 bug：
...
所以拆 Agent 不是默认选项。拆分标准是上下文冲突——不同任务需要不同上下文，塞在一起 Agent 分不清该关注什么——而不是任务数量。

**修复：** 墨简的 7 Agent 链路，每个只做一件事：
...
Agent 之间的契约是文件——上一个 Agent 的输出是下一个的输入。三条核心协议保证链路运转：

| 协议 | 规则 |
|---|---|
| 交接 | 输出必须有 frontmatter（`draft → review → approved`），`approved` 才能触发下游 |
| 决策写入 | 口头决策写入文档，否则对下一个 Agent 不存在 |
| 环境演进 | Agent 失败不重试，诊断环境缺口——失败是环境的信号，不是 Agent 的问题 |

有损传递的修复：在链路末端加入机械化闭环——`trace.sh --strict`。代码中用 `@req R1.1` 注解标记实现了哪个需求，脚本逐条核对。不靠 Agent 记忆回溯，靠脚本扫描。
...
3. 每次犯错写一条 linter rule 或在 SKILL.md 中加一个检查步骤，不是修代码
