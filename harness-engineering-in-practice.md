...
我的配图系统（[LayerAxis](https://github.com/Aryous/layeraxis-marketplace)）完整经历了这个过程。它是一个三级 Agent 流水线，为文章的每个章节设计和生成配图：
...
三个 agent 用不同的模型。creative 用 Opus——创意需要大上下文窗口保持连贯性。render 用 Haiku——只是跑脚本调 API，不需要推理能力。**模型选择匹配认知负载，而不是统一用最强的。**

从 33% 的可用率迭代到 100%。模型没有换，约束变得更精确了。

### 33%——隐喻选完没展开

6 张配图只有 2 张可用。Agent 选了隐喻（"灯塔"代表方向），但没有展开为视觉规格——灯塔多大？在画面什么位置？光束往哪照？

**Harness 修改**：在 SKILL.md 中加入隐喻展开三步法：

1. **选类型**（从隐喻库选择：容器、路径、工具、生态系统、空间）
2. **定规格**（形态、比例、位置、标注、语义标记——视觉规格，不是概念选择）
3. **验可读**（不看文章，能在 3 秒内理解核心含义吗？）

之前的流程是选完隐喻直接写提示词。中间缺了第二步——视觉规格化。灯塔多大？在画面什么位置？光束往哪照？这些问题没有答案，提示词就是模糊的。
...
早期版本用 JSON 存储中间产物。然后中文标点（`""`）导致 JSON.parse 崩溃。这个 bug 反复出现，补丁修不干净。

**Harness 修改**：把三种信息拆到匹配的格式里——YAML frontmatter 存结构化元数据（评分、生成参数），Markdown body 存创意分析（场景设计的推理过程），Markdown section 存机器可解析的提示词（`## English Prompt`）。

这个迁移教会了我一件事：**格式应该匹配认知工作的类型。** YAML 适合配置，Markdown 适合叙事推理。把两者塞进 JSON 里，两边都做不好。
...
每个 gate 是一个确定性检查点。通过才能继续，没通过就回退——没有部分通过，要么全过要么全回退。

Gate 的设计原则是**成功静默，失败出声**。验证通过不输出任何信息，只有失败时才注入 agent 上下文。这来自 HumanLayer 的教训：他们早期让 agent 跑完整测试套件，4000 行通过的测试输出涌入上下文，agent 反而开始幻觉。
