...
To find value, you *must* use an **agent**. An agent is the industry-adopted term for an LLM that can chat and invoke external behavior in a loop <sup><a href="#user-content-fn-1">1</a></sup> At a bare minimum, the agent must have the ability to: read files, execute programs, and make HTTP requests.
